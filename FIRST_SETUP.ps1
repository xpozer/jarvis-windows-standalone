...
$script:LogFile = Join-Path $script:Logs "setup.log"
...
Jv-Step "JARVIS Erstsetup startet"
Jv-Info "Root: $Root"
...
if(-not (Test-Path $VenvPython)){
  ...
  if($code -ne 0){ Jv-Fail "Python venv konnte nicht erstellt werden. ExitCode: $code" }
} else {
  Jv-Ok "Python venv vorhanden"
}

Invoke-Checked "Aktualisiere pip" $VenvPython @("-m","pip","install","--upgrade","pip") "pip.log" $Backend
...
if(-not (Test-Path (Join-Path $Dist "index.html"))){
  Jv-Fail "Frontend Build hat keine dist\index.html erzeugt. Siehe logs\npm-build.log"
}

if(Test-Path (Join-Path $Frontend "diagnose.html")){
