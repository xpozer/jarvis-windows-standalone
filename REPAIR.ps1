...
$script:LogFile = Join-Path $script:Logs "repair.log"
...
if(-not (Test-Path $VenvPython)){
  ...
  $py = Invoke-Capture -Command $VenvPython -Arguments @("--version") -TimeoutSeconds 10
  if(-not $py.Ok){
    Jv-Warn "venv Python antwortet nicht"
    $needsBackend = $true
  } else {
    Jv-Ok "venv Python OK: $($py.Output)"
  }
}

if(-not (Test-Path $DistIndex)){
  Jv-Warn "Frontend dist\index.html fehlt"
  $needsFrontend = $true
} else {
  Jv-Ok "Frontend Build vorhanden"
}

if($needsBackend){
  $pyLauncher = @(Get-PythonLauncher)
  if(Test-Path (Join-Path $Backend ".venv")){
    Jv-Warn "Entferne defekte venv"
  ...
  try {
    $pyCmd = $pyLauncher[0]
    $pyArgs = @()
    if($pyLauncher.Count -gt 1){ $pyArgs = $pyLauncher[1..($pyLauncher.Count-1)] }
    & $pyCmd @pyArgs -m venv .venv
    if($LASTEXITCODE -ne 0){ Jv-Fail "venv konnte nicht neu erstellt werden" }
  } finally { Pop-Location }
  $VenvPython = Join-Path $Backend ".venv\Scripts\python.exe"
  Invoke-Checked "Installiere Backend Dependencies" $VenvPython @("-m","pip","install","-r","requirements.txt") "pip.log" $Backend
}
...
if($needsFrontend){
  ...
  Invoke-Checked "Baue Frontend Production Build" $npm @("run","build") "npm-build.log" $Frontend
  if(-not (Test-Path $DistIndex)){ Jv-Fail "Frontend Build weiterhin defekt" }
}

if(Test-Cmd "ollama"){
  if(-not (Test-Port 11434)){
    Jv-Warn "Ollama nicht erreichbar. Starte Ollama."
    Start-Process -FilePath "ollama" -ArgumentList "serve" -WindowStyle Minimized
    Wait-Port 11434 8 | Out-Null
  }
  if(Test-Port 11434){ Jv-Ok "Ollama erreichbar" } else { Jv-Warn "Ollama weiterhin nicht erreichbar" }
} else {
  Jv-Warn "Ollama CLI fehlt. Chat braucht Ollama später separat."
}

if(-not (Test-Port 8000)){
  Jv-Step "Starte Backend für Prüfung"
  Start-Process -FilePath "cmd.exe" -ArgumentList "/k cd /d `"$Backend`" && `"$VenvPython`" main.py" -WindowStyle Minimized
  if(-not (Wait-Port 8000 20)){ Jv-Fail "Backend wurde nach Repair nicht erreichbar" }
}
Jv-Ok "Backend erreichbar"

Jv-Ok "Repair abgeschlossen"
