...
def test_lifeos_status_falls_back_to_example(monkeypatch, tmp_path):
    ...
    example_file.write_text(
        json.dumps({"version": "test", "daily_briefing": {"today_important": ["Test"]}}),
        encoding="utf-8",
    )
    monkeypatch.setattr(lifeos, "LIFEOS_PRIVATE_FILE", private_file)
    monkeypatch.setattr(lifeos, "LIFEOS_EXAMPLE_FILE", example_file)
        ...
        json.dumps(
            {
                ...
                "work_radar": {
                    "items": [
                        {
                            "id": "sap_auftrag",
                            "title": "SAP Auftrag pruefen",
                            "status": "blocked",
                            "risk": "critical",
                            "deadline": "2026-05-01",
                            "next_step": "Bestellwert klaeren",
                        },
                        {
                            "id": "fsm_buchung",
                            "title": "FSM Buchung nachziehen",
                            "status": "waiting",
                            "risk": "medium",
                            "deadline": "2026-05-03",
                            "next_step": "Nachweis suchen",
                        },
                        {
                            "id": "lnw_check",
                            "title": "LNW unterschreiben lassen",
                            "status": "open",
                            "risk": "high",
                            "deadline": "2026-05-02",
                            "next_step": "Kundenfreigabe holen",
                        },
                    ]
                },
            }
