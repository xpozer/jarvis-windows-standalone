...
def test_update_center_status_contains_safe_components(monkeypatch):
    ...
    def fake_run(args, cwd=None, timeout=8):
        ...
        if args[:2] == ["git", "remote"]:
            return update_center.CommandResult(
                True, "https://github.com/xpozer/jarvis-windows-standalone.git", "", 0
            )
    ...
    monkeypatch.setattr(
        update_center, "_ollama_tags", lambda: {"ok": True, "models": [{"name": "qwen3:8b"}]}
    )
