...
export function RuntimeControlPanel({ onSend }: Props) {
  ...
  const [workflowName, setWorkflowName] = useState("Tagesstart Demo");
  ...
  const [writeContent, setWriteContent] = useState("Hallo von JARVIS Aktionen Stufe 2.");
  ...
  async function createDemoWorkflow() {
    const clean = workflowName.trim() || "Runtime Demo Ablauf";
  ...
  const cards = useMemo(() => {
    ...
    return [
      { label: "Gedaechtnis", value: safeCount(status?.primitives?.memory?.facts), sub: "Fakten" },
      { label: "Awareness", value: safeCount(status?.primitives?.awareness?.events), sub: loop?.enabled ? "Loop aktiv" : "Events" },
      { label: "Aktionen", value: safeCount(status?.primitives?.action?.pending_approval), sub: "offen" },
      { label: "Agenten", value: safeCount(status?.primitives?.orchestration?.agents), sub: "Rollen" },
      { label: "Ablaeufe", value: safeCount(status?.workflow_runtime?.workflows), sub: "gespeichert" },
      { label: "Nebenkerne", value: safeCount(status?.workflow_runtime?.sidecars), sub: "Maschinen" },
    ];
  ...
  return (
    <section className="runtime-control-shell">
      <div className="runtime-control-header">
        <div>
          <small>LOKALER USEJARVIS KERN</small>
          <h1>JARVIS Runtime</h1>
          <p>Gedaechtnis, Awareness, Aktionen, Orchestrierung, Ablaeufe, Ziele und Nebenkerne als lokale Kontrollzentrale.</p>
        </div>
        <div className="runtime-control-actions">
          <button onClick={() => onSend("Fasse mir den aktuellen JARVIS Runtime Status zusammen")}>JARVIS FRAGEN</button>
          <button onClick={() => void captureAwareness()}>{capturing ? "ERFASST" : "KONTEXT ERFASSEN"}</button>
          <button onClick={() => void refresh()}>{loading ? "LAEDT" : "AKTUALISIEREN"}</button>
        </div>
      </div>
