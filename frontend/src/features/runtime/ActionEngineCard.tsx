...
export function ActionEngineCard({
  ...
  return (
    <section className="runtime-control-card runtime-action-card runtime-action-card-level2">
      <div className="runtime-control-card-title"><h2>Aktionszentrale</h2><span>Werkzeuge mit Stufe-2-Freigabe</span></div>
      <div className="runtime-action-toolbar">
        ...
        <button onClick={() => void onRunAction("process.list", { limit: 25 })}>PROZESSE</button>
      </div>
      <div className="runtime-action-read-grid">
        ...
        <button onClick={() => void onRunAction("filesystem.list_dir", { path: actionPath || undefined, limit: 60 })}>LISTE</button>
        <button onClick={() => void onRunAction("filesystem.read_file", { path: actionPath, max_bytes: 128000 })}>LESEN</button>
      </div>
      <div className="runtime-control-form runtime-action-path">
        <input value={actionUrl} onChange={(e) => onActionUrlChange(e.target.value)} placeholder="URL fuer Browser Aktion" />
        <button onClick={() => void onRunAction("browser.open_url", { url: actionUrl })}>URL VORBEREITEN</button>
      </div>

      <div className="runtime-action-divider"><span>Dateisystem mit Freigabe</span></div>

      <div className="runtime-control-form runtime-action-path">
        <input value={mkdirPath} onChange={(e) => onMkdirPathChange(e.target.value)} placeholder="Ordnerpfad, z.B. data/test-folder" />
        <button onClick={() => void onRunAction("filesystem.make_dir", { path: mkdirPath })}>ORDNER VORBEREITEN</button>
      </div>

      <div className="runtime-control-form runtime-action-path">
        <input value={writePath} onChange={(e) => onWritePathChange(e.target.value)} placeholder="Textdatei Pfad, z.B. data/test.txt" />
        <button onClick={() => void onRunAction("filesystem.write_text_file", { path: writePath, content: writeContent, overwrite: false })}>SCHREIBEN VORBEREITEN</button>
      </div>
      <textarea className="runtime-action-textarea" value={writeContent} onChange={(e) => onWriteContentChange(e.target.value)} placeholder="Textinhalt, max. 128 KB. Ueberschreiben ist im UI bewusst aus." />
      ...
      <div className="runtime-action-copy-grid">
        ...
        <button onClick={() => void onRunAction("filesystem.copy_file", { source: copySource, destination: copyDestination, overwrite: false })}>KOPIE VORBEREITEN</button>
      </div>

      <pre className="runtime-action-result">{actionResult ? pretty(actionResult) : "Noch kein Aktionsergebnis."}</pre>
    </section>
  );
}
