  ...
  return (
    <section className="runtime-control-card runtime-control-card-tall">
      ...
      <div className="runtime-control-form">
        ...
        <button onClick={() => void onAddFact()}>SAVE</button>
      </div>
      <div className="runtime-control-list">
        {facts.map((fact) => <article key={fact.id}><b>{fact.fact_text}</b><span>{fact.source_type} · importance {fact.importance ?? 0} · {prettyDate(fact.created_at)}</span></article>)}
